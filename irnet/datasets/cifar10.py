...
def load_cifar10(args):
    ...
    data_train = np.split(data_train, len(data_train), axis=0) # 50000 * [1, 32, 32, 3]
    ...
    data_test, dlabels_test = temp_test.data, temp_test.targets # (10000, 32, 32, 3)
    data_test = np.split(data_test, len(data_test), axis=0) # 10000 * [1, 32, 32, 3]
...
class Augmentention(Dataset):
    ...
    def __getitem__(self, index):
        if self.train_flag == True:
            each_image_w1 = self.weak_transform(self.images[index][0]) # due to np.split, change (1,32,32,3) => (32,32,3)
            each_image_w2 = self.weak_transform(self.images[index][0])
            each_image_w3 = self.weak_transform(self.images[index][0])
            each_image_s1 = self.strong_transform(self.images[index][0])
            each_image_s2 = self.strong_transform(self.images[index][0])
            each_image_s3 = self.strong_transform(self.images[index][0])
            ...
            each_image = self.test_transform(self.images[index][0])
            each_dlabel = self.dlabels[index]
